...
public class L1S implements Forgingl1 {
    ...
    public L1S(Identifier id, String group, int width, int height, DefaultedList<Ingredient> ingredients, ItemStack output) {
        this.id = id;
        this.group = group;
        this.width = width;
        this.height = height;
        ...
        for(int i = 0; i < inv.getWidth(); ++i) {
            for(int j = 0; j < inv.getHeight(); ++j) {
                ...
                if (k >= 0 && l >= 0 && k < this.width && l < this.height) {
                    if (bl) {
                        ingredient = (Ingredient)this.inputs.get(this.width - k - 1 + l * this.width);
                    } else {
                        ingredient = (Ingredient)this.inputs.get(k + l * this.width);
                    }
                }
    ...
    public static class Serializer implements RecipeSerializer<L1S> {
        public Serializer() {
        }

        public L1S read(Identifier identifier, JsonObject jsonObject) {
            String string = JsonHelper.getString(jsonObject, "group", "");
            Map<String, Ingredient> map = L1S.getComponents(JsonHelper.getObject(jsonObject, "key"));
            String[] strings =  L1S.combinePattern( L1S.getPattern(JsonHelper.getArray(jsonObject, "pattern")));
            int i = strings[0].length();
            int j = strings.length;
            DefaultedList<Ingredient> defaultedList =  L1S.getIngredients(strings, map, i, j);
            ItemStack itemStack = ShapedRecipe.getItemStack(JsonHelper.getObject(jsonObject, "result"));
            return new L1S(identifier, string, i, j, defaultedList, itemStack);
        }

        public L1S read(Identifier identifier, PacketByteBuf packetByteBuf) {
        ...
        public void write(PacketByteBuf packetByteBuf, L1S shapedRecipe) {
